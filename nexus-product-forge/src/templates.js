...
function appProject(spec) {
  ...
  const script = `const key='${spec.slug}:items',memory=new Map();let persistent=true;const storage=(()=>{try{const s=localStorage,probe='__nexus_probe__';s.setItem(probe,'1');s.removeItem(probe);return s}catch{persistent=false;return{getItem:k=>memory.has(k)?memory.get(k):null,setItem:(k,v)=>memory.set(k,String(v))}}})();let items=JSON.parse(storage.getItem(key)||'[]');const list=document.querySelector('#items'),record=document.querySelector('#record'),formEl=document.querySelector('#form'),ideaEl=document.querySelector('#idea');function draw(){list.innerHTML='';items.forEach((x,i)=>{const row=document.createElement('button');row.className='item';row.textContent=(x.done?'✓ ':'○ ')+x.text;row.onclick=()=>{items[i].done=!items[i].done;save()};list.append(row)});record.textContent=JSON.stringify({count:items.length,complete:items.filter(x=>x.done).length,storage:persistent?'localStorage':'preview-memory'},null,2)}function save(){storage.setItem(key,JSON.stringify(items));draw()}formEl.onsubmit=e=>{e.preventDefault();const text=ideaEl.value.trim();if(!text)return;items.push({text,done:false});ideaEl.value='';save()};draw();`;
